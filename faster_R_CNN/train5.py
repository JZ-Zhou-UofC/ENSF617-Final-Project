...
def train_model(train_loader, val_loader, epochs=15):
    ...
    wandb.init(project="faster-rcnn-traffic", name="roi_head_upgrade", mode="offline")
    ...
    for epoch in range(PHASE1_EPOCHS):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), "best_model_roi.pth")

    # Phase 2
    model = get_model(NUM_CLASSES, freeze_layers=True)
    model.load_state_dict(torch.load("best_model_roi.pth"))
    model.to(device)
...
# =========================
def main():
    dataset = NinjaDataset(IMG_DIR, ANN_DIR, transforms=None)
